...
async function createShiprocketOrder(order: Order, items: CartItem[], phone: string) {
    if (!order.user_id) {
        console.warn("No user ID associated with the order. Skipping Shiprocket shipment creation.");
        return;
    }
    ...
    try {
        ...
        if (profileError || !userProfile) {
            console.error('Could not fetch user profile for Shiprocket order.', profileError);
            return;
        }
        ...
        if (!shippingAddress || !shippingAddress.street || !shippingAddress.city || !shippingAddress.postal_code || !shippingAddress.country || !shippingAddress.state) {
            console.error('Incomplete shipping address for user. Skipping Shiprocket order.', userProfile);
            return; 
        }
